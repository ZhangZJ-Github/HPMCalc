...
def dataframe_of_TM_map_to_CST_readable_txt(df: pandas.DataFrame, field_name="E"):
    ...
    if field_name == "E":
        ...
        df.columns = re.split(r'\s{2,}',
                              "x [mm]           y [mm]           z [mm]       HxRe [A/m]       HxIm [A/m]       HyRe [A/m]       HyIm [A/m]       HzRe [A/m]       HzIm [A/m]", )
    else:
...
class TM_1D_extroplator:
    def __init__(self, Ez1d_df: pandas.DataFrame, omega: float):
        ...
        self.Ez1d_interpolator = scipy.interpolate.interp1d(self.zdata, self.Ez1ddata, fill_value=0.,
                                                            bounds_error=False)
