...
if __name__ == '__main__':
    ...
    def get_total_power_reflection_coeff_dB(args,
        ...
        nw_HOM_coax_line0 = [
            med.line(30, unit='mm', name="nw_HOM_coax_line0 (%d)" % (i + 1)).interpolate(
                resampling_frequency)
            for i, med in enumerate(CoaxToRect.medias)]
        nw_HOM_coax_line1 = [
            med.line(line1_length_mm, unit='mm', name="nw_HOM_coax_line1 (%d)" % (i + 1)).interpolate(
                resampling_frequency)
            for i, med in enumerate(CoaxToRect.medias)]
        ...
        connexion = [
                        ...
                        for i, line in enumerate(nw_HOM_coax_line0)
                    ] + [
                        [(line, 1), (CoaxWithSuppRods.nw, 0 + i)]
                        for i, line in enumerate(nw_HOM_coax_line0)
                    ] + [
                        [(CoaxWithSuppRods.nw, 5 + i), (line, 0)] for i, line in enumerate(nw_HOM_coax_line1)
                    ] + [
                        [(line, 1), (CoaxToRect.nw, 0 + i)] for i, line in enumerate(nw_HOM_coax_line1)
                    ]
