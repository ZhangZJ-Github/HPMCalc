...
if __name__ == '__main__':
    ...
    CoaxWithSuppRods2_nw = CoaxWithSuppRods.nw.copy()
    CoaxWithSuppRods2_nw.name = "CoaxWithSuppRods2"
    ...
    def get_total_power_reflection_coeff(args,
        ...
        line1_length_mm,line2_length_mm = args
        data_to_record = {}
        nw_HOM_coax_line = [
            med.line(line1_length_mm, unit='mm', name="nw_HOM_coax_line, mode %d" % (i + 1)).interpolate(resampling_frequency)
            for i, med in enumerate(CoaxToRect.medias)]
        nw_HOM_coax_line2 = [
            med.line(line2_length_mm, unit='mm', name="nw_HOM_coax_line2, mode %d" % (i + 1)).interpolate(resampling_frequency)
            for i, med in enumerate(CoaxToRect.medias)]
        ...
        connexion = [
                        ...
                        [(line, 1), (CoaxWithSuppRods2_nw, 0 + i)] for i, line in enumerate(nw_HOM_coax_line)
                    ]+[
                        [(CoaxWithSuppRods2_nw, 5 + i),(line, 0)] for i, line in enumerate(nw_HOM_coax_line2)
                    ] + [
                        [(line, 1), (CoaxToRect.nw, 0 + i)] for i, line in enumerate(nw_HOM_coax_line2)
                    ]
