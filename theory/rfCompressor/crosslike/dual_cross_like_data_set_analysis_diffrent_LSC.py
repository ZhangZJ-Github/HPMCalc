...
def __get_beta_l_to_min_or_max_power_output(cross_like: CrossLikeNetwork,
    ...
    __get_output_signals = lambda beta_l_delay_shorts: __get_output_signal_of_the_network_connects_with_delay_shorts(
        cross_like.nw_cross,
        port_ids_mismatched, [beta_l_delay_shorts[0], beta_l_delay_shorts[1], beta_l_delay_shorts[2], ],
        index_of_port_to_input_signal, 1 + 0j, f_target
    )
    get_power_out_ratio = lambda beta_l_delay_shorts: numpy.sum(
        numpy.abs(__get_output_signals(beta_l_delay_shorts)[port_ids_out]) ** 2, axis=-1)

    min_res = minimize(lambda beta_l_delay_shorts :get_power_out_ratio(numpy.array((beta_l_delay_shorts[0],beta_l_delay_shorts[1],beta_l_delay_shorts[1]))),
                       x0=(beta_l_range_for_finding_min[:, 0] + beta_l_range_for_finding_min[:, 1]) / 2,
                       bounds=beta_l_range_for_finding_min,
                       # tol = 0.01/19
                       )
    ...
    logger.info(min_res)
    ratio_power_outgoing_side_arms =numpy.abs(__get_output_signals(numpy.array((*min_res.x,min_res.x[1])))[port_ids_to_SC]) ** 2
    ratio_power_outgoing_side_arm_max = numpy.max(ratio_power_outgoing_side_arms, axis=-1)

    logger.info(
        "Power passing forward one of the side arms = %s%%" % (ratio_power_outgoing_side_arms * 100))
    P0 = 1e9
    P = P0 * ratio_power_outgoing_side_arm_max
    ...
    logger.info(
        "When the power output is zero, maximum E field inside the side arm is %.2e V/m, which is %.2f%% of that inside the ESWG (%.2e V/m)\n(assuming the power input from port 1 is 1 GW)" % (
            ...
            max_E_inside_switch_cav_when_no_out_to_max_E_inside_ESWG * 100, max_E_inside_ESWG,))
    dL_SC = 1e-3
    beta = beta_side_arm_interpolator(f_target_GHz) * dL_SC
    f2 = lambda beta_l_SC: - get_power_out_ratio([min_res.x[0], beta_l_SC[0],beta_l_SC[0]+ beta])
    max_res = minimize(f2,
                       x0=(beta_l_range_for_finding_max[:, 0] + beta_l_range_for_finding_max[:, 1]) / 2,
                       bounds=beta_l_range_for_finding_max,  # tol = 0.01/19
                       )
    logger.info("%s"%(numpy.abs( __get_output_signal_of_the_network_connects_with_delay_shorts(
            cross_like.nw_cross, port_ids_mismatched, numpy.array([min_res.x[0],max_res.x[0],max_res.x[0] + beta]),
    ...
    l_SC_max_out2 = (max_res.x[0] +beta) / beta_side_arm_interpolator(f_target_GHz)
    ...
    if __name__ == '__main__':
        ...
        _S_square = 1 - numpy.abs(vectf_calculate_signals_outgoing(
            numpy.array((beta_thru_arm_interpolator(f_GHz) * l_thru, beta_side_arm_interpolator(f_GHz) * l_SC_min_out,
                         beta_side_arm_interpolator(f_GHz) * l_SC_min_out,)).T,
            f
        )[:, 0]) ** 2
        ...
        plt.plot(f / 1e9, 1 - numpy.abs(vectf_calculate_signals_outgoing(
            numpy.array((beta_thru_arm_interpolator(f_GHz) * l_thru, beta_side_arm_interpolator(f_GHz) * l_SC_max_out,
                         beta_side_arm_interpolator(f_GHz) * l_SC_max_out2,)).T,
            f
        )[:, 0])**2, )
        plt.ylim(0, 1)
...
if __name__ == '__main__':
    s_interpolator_on_dataset, z0_interpolator_on_dataset, data_set = get_interpolator_on_dataset(
        r"E:\CSTprojects\rfCompressor\cascadedHT\why_2hole\HplaneDualCrossLike.cst", ["SCcenterDz", 'OutDz1', ])
    ...
    map_from_port_id_to_matched_waveguide = dict(
        zip(port_ids_mismatched, [rect_WG_to_thru_arm, rect_WG_to_side_arms, rect_WG_to_side_arms]))
    plt.ioff()
    # for i, (SCcenterDz, OutDz1) in enumerate(
    #         numpy.array(numpy.meshgrid(numpy.unique(s_interpolator_on_dataset.tri.points[:, 0])[::2],
    #                                    numpy.unique(s_interpolator_on_dataset.tri.points[:, 1])[::3], )).transpose(
    #             (1, 2, 0)).reshape((-1, 2))):
    DL_SC = 1.0e-3

    for i, (SCcenterDz, OutDz1) in [[0, (40,1e-3)]]:

        str_indexing_parameter = "SCcenterDz = %.0f mm, OutDz1 = %.0f mm dL_SC = %.1f mm" % (SCcenterDz, OutDz1,DL_SC * 1e3)
        logger.info(str_indexing_parameter)
        nw_cross = skrf.Network(frequency=skrf_frequency,
                                s=s_interpolator_on_dataset((SCcenterDz, OutDz1)), z0=z0
                                )
        ...
        vectf__get_output_signal_of_the_network_connects_with_delay_shorts = numpy.vectorize(
            lambda beta_l_thru, beta_l_SC1, beta_l_SC2: __get_output_signal_of_the_network_connects_with_delay_shorts(
                nw_cross,
                port_ids_mismatched,
                [beta_l_thru,
                 beta_l_SC1,
        ...
        S_allports_0 = vectf__get_output_signal_of_the_network_connects_with_delay_shorts(BETA_L_THRU, BETA_L_SC,
                                                                                          BETA_L_SC+beta_side_arm_interpolator(f_target_GHz) *DL_SC)
    ...
    res = __get_beta_l_to_min_or_max_power_output(
        CrossLikeNetwork(
            skrf.Network(frequency=skrf_frequency,
                         s=s_interpolator_on_dataset((40, 1.0e-03)),
        ...
        port_ids_out,
        numpy.array([[0.74,0.76], [0.3, 0.7]]) * numpy.pi,
        numpy.array([[-0.3, 0.3],
                     # [-0.3,0.3]
                     ]) * numpy.pi,
    )
