...
def __get_beta_l_to_min_power_output(
    ...
    max_E_inside_switch_cav_when_no_out = 4 * (numpy.abs(z0_WG_side_arm_interpolator(f_target_GHz)) * P / (
            rect_WG_to_side_arms_a * rect_WG_to_side_arms_b)) ** 0.5

    max_E_inside_ESWG = 4 * (numpy.abs(z0_WG_thru_arm_interpolator(f_target_GHz)) * P0 / (
            rect_WG_to_thru_arm_a * rect_WG_to_thru_arm_b)) ** 0.5
...
if __name__ == '__main__':
    s_interpolator_on_dataset, z0_interpolator_on_dataset, data_set = get_interpolator_on_dataset(
        r"E:\CSTprojects\rfCompressor\cascadedHT\why_2hole\HplaneCrossLikeCopper.cst", [  # "SCcenterDz",
            'OutDz1', ])
    ...
    z0 = example_data['z0']
    gamma_interps = example_data['gamma_interp']
    # N_considered_modes = 3
    electric_conductivity = 5.8e7# Copper
    from skrf.media.media import  DefinedGammaZ0
    GHz = 1e9
    i = 0
    rect_WG_to_thru_arm = DefinedGammaZ0(
        skrf_frequency,z0_port= z0[:,i],z0 = z0[:,i],
        gamma = gamma_interps[i](skrf_frequency.f/GHz)
    )
    rect_WG_to_thru_arm_a , rect_WG_to_thru_arm_b = 30e-3, 15e-3

    i =  2
    rect_WG_to_side_arms =DefinedGammaZ0(
        skrf_frequency,z0_port= z0[:,i],z0 = z0[:,i],
        gamma = gamma_interps[i](skrf_frequency.f/GHz)
    )
    rect_WG_to_side_arms_a , rect_WG_to_side_arms_b = 30e-3,15e-3

    i =  3
    rect_WG_to_outlet =DefinedGammaZ0(
        skrf_frequency,z0_port= z0[:,i],z0 = z0[:,i],
        gamma = gamma_interps[i](skrf_frequency.f/GHz)
    )
    if 0:
        rect_WG_to_thru_arm = RectangularWaveguide(frequency=skrf_frequency,
                                               ...
                                               a=30e-3, b=15e-3, mode_type='te', m=1, n=0,#rho = 1/electric_conductivity

                                               )

    # rect_WG_to_thru_arm.al
    ...
    # rect_WGs = [rect_WG_to_thru_arm, rect_WG_to_side_arms, rect_WG_to_side_arms]
        rect_WG_to_outlet = RectangularWaveguide(frequency=skrf_frequency,
                                             # z0_port = nw.z0[:,__name_map_from_CST_to_skrf[2]],
                                             a=18e-3, b=15e-3, mode_type='te', m=1, n=0,rho = 1/electric_conductivity)
    ...
    res = __get_beta_l_to_min_or_max_power_output(
        CrossLikeNetwork(
            skrf.Network(frequency=skrf_frequency,
                         s=s_interpolator_on_dataset(( 5#1.0e-03
                                                       )),
    ...
    nw_charging=                CrossLikeNetwork(
            skrf.Network(frequency=skrf_frequency,
                         ...
                         #                 )).reshape((-1, 6))
                         z0=z0
                         ), map_from_port_id_to_matched_waveguide).get_nw_connected_with_delay_shorts({
        1:res[0].x[0],
        3:res[0].x[1],
    })[-1]

    nw_discharging=        CrossLikeNetwork(
            skrf.Network(frequency=skrf_frequency,
                         s=s_interpolator_on_dataset(( 5#1.0e-03
                                                       )),
                         # z0=numpy.array((rect_WG_to_thru_arm.z0, rect_WG_to_thru_arm.z0,
    ...
    plt.figure()
    nw_charging.plot_s_mag()
    plt.figure()
    nw_charging.plot_s_db()



    plt.figure()
    nw_discharging.plot_s_mag()
