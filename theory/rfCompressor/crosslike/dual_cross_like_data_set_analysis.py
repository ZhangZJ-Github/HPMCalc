...
def __get_beta_l_to_min_or_max_power_output(cross_like: CrossLikeNetwork,
    ...
    logger.info(min_res)
    ratio_power_outgoing_side_arms =numpy.abs(__get_output_signals(min_res.x)[port_ids_to_SC]) ** 2
    ...
    logger.info("%s"%(numpy.abs( __get_output_signal_of_the_network_connects_with_delay_shorts(
            cross_like.nw_cross, port_ids_mismatched, numpy.array([min_res.x[0],max_res.x[0],max_res.x[0]]),
    ...
    if __name__ == '__main__':
        ...
        plt.plot(f / 1e9, 1 - numpy.abs(vectf_calculate_signals_outgoing(
            numpy.array((beta_thru_arm_interpolator(f_GHz) * l_thru, beta_side_arm_interpolator(f_GHz) * l_SC_max_out,
                         beta_side_arm_interpolator(f_GHz) * l_SC_max_out,)).T,
            f
        )[:, 0])**2, )
        plt.ylim(0, 1)
...
if __name__ == '__main__':
    ...
    #             (1, 2, 0)).reshape((-1, 2))):
    for i, (SCcenterDz, OutDz1) in [[0, (38, 2)]]:
        str_indexing_parameter = "SCcenterDz = %.0f mm, OutDz1 = %.0f mm" % (SCcenterDz, OutDz1)
    ...
    res = __get_beta_l_to_min_or_max_power_output(
        CrossLikeNetwork(
            skrf.Network(frequency=skrf_frequency,
                         s=s_interpolator_on_dataset((32, 1.0e-03)),
