...
def get_interpolator_on_dataset(cst_proj_path: str, indexing_parameter_names: typing.List[str]):
    ...
    s_interpolator_on_dataset = Interpolator(_indexing_parameters,
                                             numpy.array([data_set[run_id]["s"] for run_id in data_set.keys()]),**kwargs_for_interpolator
                                             )
    z0_interpolator_on_dataset = Interpolator(_indexing_parameters,
                                              numpy.array([data_set[run_id]["z0"] for run_id in data_set.keys()]),**kwargs_for_interpolator
                                              )
...
if __name__ == '__main__':
    s_interpolator_on_dataset, z0_interpolator_on_dataset, data_set = get_interpolator_on_dataset(
        r"E:\CSTprojects\rfCompressor\cascadedHT\why_2hole\HplaneCrossLikeHighMode.cst", ['OutDz1', ])
    example_data = data_set[list(data_set.keys())[0]]
    f = example_data['f']
    skrf_frequency = skrf.Frequency.from_f(f, unit='GHz')
    z0 = example_data['z0']
    ...
    rect_WG_to_thru_arm = [RectangularWaveguide(frequency=skrf_frequency,
                                               ...
                                               a=30e-3, b=15e-3, mode_type='te', m=i, n=0) for i in range(1,1+N_considered_modes)]
    # 开关腔的雏形波导
    rect_WG_to_side_arms = RectangularWaveguide(frequency=skrf_frequency,
                                                a=30e-3, b=15e-3, mode_type='te', m=1, n=0)

    for OutDz1 in s_interpolator_on_dataset.x:
        nw_cross = skrf.Network(frequency=skrf_frequency,
                                s=s_interpolator_on_dataset(OutDz1), z0=z0)
        line_to_connect_two_cross  = [rect_WG.line(d=32-60 *2,unit= 'mm' )for rect_WG in rect_WG_to_thru_arm]
        # multi_mode_line_to_connect_two_cross =
        nw_dual_cross_s = nw_cross.s.copy()
        for i in range(N_considered_modes):
            logger.info(i)
            nw_dual_cross_s = skrf.connect_s(nw_dual_cross_s,3,line_to_connect_two_cross[i].s,0, )
        aaaaaaaaaa



        nw_cross_load_on_thru_arm = skrf.connect_s(nw_cross.s,)

    aaaaaaaaaaa
    proj_3D: cst.results.ProjectFile = cst.results.ProjectFile(
        r"E:\CSTprojects\rfCompressor\cascadedHT\why_2hole\HplaneDualCrossLike.cst",
        allow_interactive=True)
    S = {}
    key_data = 'data'
    key_interpolator = "interpolator"
    ...
    vectf_calculate_signals_outgoing = numpy.vectorize(
        lambda beta_r_thru_arm, beta_r_side_arms: calculate_signals_outgoing(S_matrix_at_f_target, [1, 4, 5],
                                                                             numpy.array([
                                                                                 1 - 2 / (1j * numpy.tan(
                                                                                     beta_r_thru_arm) + 1),
                                                                                 1 - 2 / (1j * numpy.tan(
    ...
    TOTAL_OUTs = (numpy.abs(SIGNAL_OUTs[:, :, 2]) ** 2 + numpy.abs(SIGNAL_OUTs[:, :, 3]) ** 2) ** 0.5
    ...
    def _get_nws(beta_r_thru_arm, beta_r_side_arms):
        nw2 = skrf.connect(nw, 1, rect_WG_to_thru_arm.delay_short(d=beta_r_thru_arm, unit='rad'), 0)
        nw3 = skrf.connect(nw2, 3, rect_WG_to_side_arms.delay_short(d=beta_r_side_arms, unit='rad'), 0)
        nw4 = skrf.connect(nw3, 3, rect_WG_to_side_arms.delay_short(d=beta_r_side_arms, unit='rad'), 0)
        return nw2, nw3, nw4


    def _get_total_leakage(beta_r_thru_arm, beta_r_side_arms):
        nw2, nw3, nw4 = _get_nws(beta_r_thru_arm, beta_r_side_arms)
        return (numpy.abs(nw4.interpolate([f_target]).s[0, 1, 0]) ** 2 + numpy.abs(
            nw4.interpolate([f_target]).s[0, 2, 0]) ** 2) ** 0.5


    get_total_leakage = numpy.vectorize(_get_total_leakage, )
    d_leakage = 0.05
    if 0:
        LEAKAGES = get_total_leakage(BETA_R_THU_ARM, BETA_R_SIDE_ARM)
        plt.figure(figsize=(4, 3), constrained_layout=True)
        # nw3.plot_s_mag()
        cf = plt.contourf(BETA_R_THU_ARM / numpy.pi, BETA_R_SIDE_ARM / numpy.pi, numpy.abs(LEAKAGES),
    ...
    def get_beta_r_side_arm_to_minimize_and_maximize_o3(beta_r_thru_arm):
        # beta_r2 = 0
        nw2 = skrf.connect(nw, 1, rect_WG_to_thru_arm.delay_short(d=beta_r_thru_arm, unit='rad'), 0)
        ratio_power_outgoing_side_arm = numpy.max(numpy.abs(nw2.interpolate([f_target]).s[0, [3, 4], 0]) ** 2)
        logger.info(
        ...
        res = minimize(lambda beta_r_side_arms: numpy.sum(
            numpy.abs(vectf_calculate_signals_outgoing(beta_r_thru_arm, beta_r_side_arms[0])[[2, 3]]) ** 2),
                       [0, ], )
        ...
        nw_minimize_power_out = skrf.connect(skrf.connect(nw2, 3, side_arm_delay_short_min_out, 0), 3,
        ...
        nw_maximize_power_out = skrf.connect(skrf.connect(nw2, 3, side_arm_delay_short_max_out, 0), 3,
    ...
    axs[1].plot(beta_r_thru_arm / numpy.pi, [10 * numpy.log10(
        numpy.sum(numpy.abs(arr_nw_minimize_power_out[i].interpolate([f_target]).s[0, [1, 2], 0]) ** 2)) for i in
    ...
    for i, beta_r2 in enumerate(beta_r_thru_arm):
        S_out_port1[i, :] = arr_nw_maximize_power_out[i].s[:, [1, 2], 0]
    ...
    (beta_r_side_arms_to_minimize_out, beta_r_side_arms_to_maximize_out,
     nw_minimize_power_out, nw_maximize_power_out,
     max_E_inside_switch_cav_when_no_out,
     max_E_inside_switch_cav_when_no_out_to_max_E_inside_ESWG) = get_beta_r_side_arm_to_minimize_and_maximize_o3(
        best_beta_r_thru_arm)
    # best_side_arm_min_out = rect_WG_to_thru_arm.delay_short(d=best_beta_r_side_arm_min_out * numpy.pi, unit='rad')
    plt.figure()
    plt.plot(nw_minimize_power_out.f / 1e9, 10 * numpy.log10(1 - numpy.abs(nw_minimize_power_out.s[:, 0, 0]) ** 2),
             label="my calc")
