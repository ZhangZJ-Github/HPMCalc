...
if __name__ == '__main__':
    ...
    vectf_calculate_signals_outgoing = numpy.vectorize(
        lambda beta_r_thru_arm, beta_r_side_arms: calculate_signals_outgoing(
            S_matrix_at_f_target, [1, 3],
            numpy.array([1 - 2 / (1j * numpy.tan(beta_r_thru_arm) + 1),
                         1 - 2 / (1j * numpy.tan(beta_r_side_arms) + 1),
                         # 1 - 2 / (1j * numpy.tan(beta_r_side_arms) + 1),
                         ],
                        complex),
            0,
    ...
    def get_beta_r_side_arm_to_minimize_and_maximize_o3(beta_r_thru_arm):
        ...
        ratio_power_outgoing_side_arm = numpy.max(numpy.abs(nw2.interpolate([f_target]).s[0, [2], 0]) ** 2)
        logger.info(
        ...
        nw_minimize_power_out = skrf.connect(nw2, 2, side_arm_delay_short_min_out, 0)
        ...
        nw_maximize_power_out =  skrf.connect(nw2, 2, side_arm_delay_short_max_out, 0)
        # plt.figure()
    ...
    S_out_port1 = numpy.zeros((*FREQS_GHz.shape, 1), dtype=complex)
    ...
    best_beta_r_thru_arm = 0.3 * numpy.pi
