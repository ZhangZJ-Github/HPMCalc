...
# cst_proj_path = r"E:\CSTprojects\rfCompressor\cascadedHT\SES_switch_1-1.WiderSwitchCav.paramsweep.cst"
cst_proj_path = r"E:\CSTprojects\rfCompressor\cascadedHT\SES_switch_1-1.WiderSwitchCav.paramsweep.cst"
...
gamma_3 = numpy.array(
    proj_charging.get_3d().get_result_item('1D Results\\Port Information\\Gamma\\3(1)', default_runid).get_data())
gamma_2 = numpy.array(
    proj_charging.get_3d().get_result_item('1D Results\\Port Information\\Gamma\\2(1)', default_runid).get_data())

Z_ref3 = numpy.array(
    (proj.get_3d().get_result_item('1D Results\\Reference Impedance\\ZRef 3(1)', default_runid)).get_data())
...
for run_id in run_ids:
    try:
        S33_3D = numpy.array(
            proj.get_3d().get_result_item('1D Results\\S-Parameters\\S3,3', run_id=run_id, ).get_data())
        S23_3D = numpy.array(
            proj.get_3d().get_result_item('1D Results\\S-Parameters\\S2,3', run_id=run_id, ).get_data())
...
def cascade_WG_and_nw(nw_WG: skrf.Network, nw: skrf.Network) -> skrf.Network:
    ...
    s[:, 1, 1] = s[:, 0, 0] = nw_WG_resampled.s[:, 0, 0] + nw.s[:, 0, 0] * nw_WG_resampled.s[:, 0,
                                                                           1] * nw_WG_resampled.s[:, 1, 0]
...
for run_id in data:
    ...
    if True:
        nw_discharging = \
            get_network_from_CST_S_data(data[run_id][key_Sparameters][[key_freq, key_S33, key_Zref3]].values,
        ...
        compressor = Compressor(
            OM_2port(
                S_param_network(numpy.vstack((nw_charging_core.f, nw_charging_core.s[:, 0, 0])).T, impulse_duration,
                                impulse_dt),
                S_param_network(numpy.vstack((nw_charging_core.f, nw_charging_core.s[:, 1, 0])).T, impulse_duration,
                                impulse_dt)),
            OM_2port(
                S_param_network(numpy.vstack((nw_discharging_core.f, nw_discharging_core.s[:, 0, 0])).T,
                                impulse_duration,
                                impulse_dt),
                S_param_network(numpy.vstack((nw_discharging_core.f, nw_discharging_core.s[:, 1, 0])).T,
                                impulse_duration,
                                impulse_dt)), )
...
df_to_plot = pandas.concat([df_to_plot,
                            ...
                            ) if cst_proj_path != 'E:\\CSTprojects\\rfCompressor\\cascadedHT\\SES_switch_1-1.WiderSwitchCav.paramsweep.cst' else
                                             numpy.array(numpy.meshgrid(numpy.linspace(0, 18, 10),
                                                                        numpy.linspace(25, 33.5, 10))).transpose(
                                                 (1, 2, 0)).reshape((-1, 2)), columns=[key_zoff, key_yoff]), ], axis=0,
                           ignore_index=True)
...
zoff0, yoff0 = z_short_terminal * 1e3 - proj_charging.get_3d().get_parameter_combination(0)["Dz3"] - (
    28 if cst_proj_path != 'E:\\CSTprojects\\rfCompressor\\cascadedHT\\SES_switch_1-1.WiderSwitchCav.paramsweep.cst' else 28 * 2) / 2, -13.4
...
for df_seq in df_seqs:
    ...
    f1 = lambda df_seq: df_seq[
        key_eta_max_of_core_structure]  # 1 / (1 - df_to_plot[key_power_balance] ** 2)  # df_seq["tau"]
...
for i in querieds.index:
    ...
    plt.plot(df_o23[trusted_index][key_time],
             (df_o23[trusted_index][key_interpolated_periodic_avg_square].values * 2) ** 0.5,
             # lw = 0.05
             label='simulated'
             )
    ...
    plt.plot(data[run_id][key_Sparameters][key_freq], numpy.abs(data[run_id][key_Sparameters][key_S33]),
             label="$|S_{1,1}|$")
    plt.plot(data[run_id][key_Sparameters][key_freq], numpy.abs(data[run_id][key_Sparameters][key_S23]),
             label="$|S_{2,1}|$")
    plt.legend()
    plt.xlim(9, 9.6)
    plt.ylim(0, 1)
...
for ii, cst_proj_path in enumerate([
    ...
    cf1 = plot_contour(df_to_plot, lambda df_to_plot: df_to_plot[key_S23], axs[1, ii], levels=numpy.linspace(0, 1, 20)
                       # levels1
                       )
    # f1 = lambda df_seq: df_seq[key_I_pe]
    # cf2 = plot_contour(df_seq, f1, axs[1], levels= levels1)

    cf2 = plot_contour(df_to_plot, lambda df_to_plot: df_to_plot[key_eta_max_of_core_structure], axs[2, ii],
                       levels=numpy.linspace(0, 0.5, 20)
                       # numpy.linspace(min(f1(df_to_plot)), max(f1(df_to_plot)), 100)
                       )
    cf3 = plot_contour(df_to_plot, lambda df_to_plot: df_to_plot[key_G_of_core_structure], axs[3, ii],
                       levels=numpy.linspace(0, 400, 20)
                       # numpy.linspace(min(f1(df_to_plot)), max(f1(df_to_plot)), 100)
                       )
