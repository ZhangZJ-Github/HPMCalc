...
for i, Dt_left in enumerate(Dt_lefts):
    ...
    eta_OM_max.append(2 * df_o23[key_interpolated_periodic_avg_square].max())
    # plt.figure()
...
for i,df_o23_ in enumerate(recorded_o23):
    if i%2==0 or Dt_lefts[i]==2e-9 :
        # plt.plot(df_o23_[0],(numpy.abs(df_o23_[key_complex])**2),label = '$\Delta t$ = %.1f ns ($L_1$ = %.2f m)'%(Dt_lefts[i]/1e-9, Dt_lefts[i]*v_g/2))
        plt.plot(df_o23_[0],(numpy.abs(df_o23_[key_interpolated_periodic_avg_square])*2),label = '$\Delta t$ = %.1f ns ($L_1$ = %.2f m)'%(Dt_lefts[i]/1e-9, Dt_lefts[i]*v_g/2))
...
L_OM = (299.77-200)*1e-3


G_cav_peak = G_OM*L_OM/v_g/(Dt_lefts+L_OM/v_g)
...
axs[0].plot(1e9*Dt_lefts, eta_OM_max,'.-',label = "$\eta_{OM, peak}$")
axs[0].plot(1e9*Dt_lefts, _func_eta_OM_peak(Dt_lefts,eta_inf, tau, Dt_right),'--',label = r"$%.3f [1-exp(-\frac{t+ %.2f\ \rm{ns}}{%.2f\ \rm{ns}})]$"%(eta_inf, Dt_right*1e9, tau*1e9,))
...
axs[1].plot(1e9*Dt_lefts, G_cav_peak,label = '$G_{cav,peak}$')
axs[2].plot(1e9*Dt_lefts, eta_OM_max*G_cav_peak,label  = "$G_{OM,peak}$")
