...
# cst_proj_path = r"E:\CSTprojects\rfCompressor\cascadedHT\single_coupling_hole\SES_trivial8.wider_switchCav.cst"
...
fig,axs = plt.subplots(3,1, figsize=(5,6),constrained_layout = True, sharex = True, sharey= True)
ns = 1e-9
i = 0
axs[i].plot(df_o23[0]/ns,numpy.abs( df_o23[key_complex])**2,label = r"convolved ($\eta_{tot}$)",lw = 3)
#
# plt.figure()
#
...
(tau, t0 , eta_inf), cov = curve_fit(func, df_o23[0][trusted_index].values,(numpy.abs(df_o23[key_complex][trusted_index])**2).values, p0 = [2.5e-9, 2e-9, 0.762])
axs[i].plot(ts_/ns, func(ts_, tau, t0 , eta_inf),'--', label = r"fitted: $%.2f \left[1-\exp{\left(-\frac{t-%.2f [ns]} {%.2f [ns]}\right)}\right]^2$"%(eta_inf,  t0/ns, tau/ns))
axs[i].legend()
axs[i].set_xlim(ts_[0]/ns, ts_[-1]/ns)

# axs[i].xlabel('time (ns)')
# axs[i].ylabel('power extraction efficiency')
# plt.savefig("transient_PEE.svg")
...
i=  2
axs[i].plot(df_o23[0]/ns,numpy.abs( df_o23[key_complex])**2+ numpy.abs( df_o43[key_complex])**2,label = r"convolved ($\eta_{tot}$)",lw = 3
            )
...
axs[i].plot(ts_/ns, func(ts_, tau, t0 , eta_inf),'--', label = r"fitted: $%.2f \left[1-\exp{\left(-\frac{t-%.2f [ns]} {%.2f [ns]}\right)}\right]^2$"%(eta_inf,  t0/ns, tau/ns))
axs[i].plot(df_o23[0]/ns, numpy.abs( df_o23[key_complex])**2,label = r"convolved ($\eta_2$)",#ls = ':'
         )
axs[i].plot(df_o23[0]/ns,  numpy.abs( df_o43[key_complex])**2,label = "convolved ($\eta_3$)",#ls = ':'
         )
axs[i].legend()
...
fig.supylabel('power extraction efficiency')
plt.savefig("typical_transient_response.svg")