...
if __name__ == '__main__':
    ...
    ey_1_zone1_at_z1 = numpy.zeros(xs.shape)

    ey_1_zone1_at_z1 [(numpy.abs(xs) < a1/ 2)] =get_e_m(xs  [(numpy.abs(xs) < a1/ 2)],z1, a1 ,1, f0)
    # 需要考虑的模式数
    max_m = xs.shape[0]//3
    M = numpy.matrix([*[get_e_m(xs,z1,a2,m,f0,+1) for m in range(1, 1+max_m)],
                     -get_e_m(xs ,z1, a1,1,f0,-1) ]).T

    res = (numpy.linalg.pinv(M) * numpy.matrix([ey_1_zone1_at_z1]).T).A.ravel()
    ...
    index_of_zone1 = (numpy.abs(X) < a1/ 2) & (Z < z1)
    index_of_zone2 =  (Z >= z1)
    ...
    for phi in numpy.linspace(0,360,36):
        ...
        cf = plt.contourf(X / 1e-3, Z / 1e-3,
                          ...
                          ,
                          cmap=plt.get_cmap('jet'), levels = numpy.linspace(-emax, emax , 20))
        plt.colorbar(cf)
