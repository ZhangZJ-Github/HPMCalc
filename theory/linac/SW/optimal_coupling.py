...
if __name__ == '__main__':
    ...
    L = 700e-3#303e-3#315e-3
    freq = 9.3e9
    adjacent_mode_separation_Hz = 10e6
    phase_advance_each_cell = numpy.pi
    N_cells = int(L/(C.c /freq * phase_advance_each_cell/(2*numpy.pi)))
    logger.info("No. of cells = %d"%(N_cells))
    estimated_k = (1/(1- adjacent_mode_separation_Hz / freq)**2- 1 ) / numpy.cos((1/2. +1/(2*N_cells-1)) *numpy.pi)
    logger.info("所需的腔间耦合系数 k = %.5f%%"%(estimated_k/1e-2))
    ...
    logger.info("Pwall = %.8f MW, Pext = %.8f MW, Pbeam = %.8f MW"%(
